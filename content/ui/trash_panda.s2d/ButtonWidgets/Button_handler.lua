...
local handler = {
    ...
    clicked = function()
        ...
        local widget = scaleform.Actor.component_by_name(input, "WidgetBase")
        local widgetApi = scaleform.ScriptComponent.script_results(widget)
        local name = widgetApi.getText()
        ...
        scaleform.AnimationComponent.goto_label(animation, "waiting")
        
        if GameModes then
            GameState.game_mode = GameModes.lobby
        end
